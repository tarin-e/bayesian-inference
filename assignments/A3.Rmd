...
### 2c) The hierarchical model’s prior has dependence between σ and the true mass parameters.
Modify the model so that it expresses exactly the same prior assumptions, but the the prior has
independence between all stochastic nodes. Hint: This is the ‘pre-whitening’ idea that we saw
for the starling ANOVA model.
...
2d) Run either version of the hierarchical model for a lot of iterations and summarise the
posterior distributions for mu, sigma, and true mass[1] using any summaries you think are
appropriate.
...
hist(results_hier$sigma, breaks = 100, main = "Histogram of mu", 
     xlab = "mu", ylab = "Frequency", col = colors)
```
...
hist_results <- hist(results_hier$`true_mass[1]`, breaks = 100)

breaks <- hist_results$breaks
colors <- ifelse(breaks >= cred[1] & breaks <= cred[2], "red", "skyblue")

hist(results_hier$`true_mass[1]`, breaks = 100, main = "Histogram of mu", 
     xlab = "mu", ylab = "Frequency", col = colors)
```
